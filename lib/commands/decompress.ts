...
function formatAvailableBlocksMessage(availableTargets: CompressionTarget[]): string {
    ...
    lines.push("Usage: /better-compact decompress <n>")
    lines.push("")
...
export async function handleDecompressCommand(ctx: DecompressCommandContext): Promise<void> {
    ...
    if (args.length > 1) {
        await sendIgnoredMessage(
            client,
            sessionId,
            "Invalid arguments. Usage: /better-compact decompress <n>",
            params,
            logger,
        )
        return
    ...
    if (targetBlockId === null) {
        await sendIgnoredMessage(
            client,
            sessionId,
            `Please enter a compression number. Example: /better-compact decompress 2`,
            params,
            logger,
        )
        return
