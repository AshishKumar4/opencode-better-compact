...
function formatAvailableBlocksMessage(availableTargets: CompressionTarget[]): string {
    ...
    lines.push("Usage: /better-compact recompress <n>")
    lines.push("")
...
export async function handleRecompressCommand(ctx: RecompressCommandContext): Promise<void> {
    ...
    if (args.length > 1) {
        await sendIgnoredMessage(
            client,
            sessionId,
            "Invalid arguments. Usage: /better-compact recompress <n>",
            params,
            logger,
        )
        return
    ...
    if (targetBlockId === null) {
        await sendIgnoredMessage(
            client,
            sessionId,
            `Please enter a compression number. Example: /better-compact recompress 2`,
            params,
            logger,
        )
        return
